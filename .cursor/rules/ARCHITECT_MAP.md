...
- **Aula AI assistant (per-message + per-channel)** — dedykowany Edge endpoint [api/aula-ai.ts](api/aula-ai.ts) (NIE `api/chat.ts`) z 4 taskami: `summarize_channel`, `explain_message`, `simplify_message`, `translate_message`. Persona + per-task prompts w [src/lib/aulaAiPrompts.ts](src/lib/aulaAiPrompts.ts) (shared client+server — endpoint importuje przez relative `../src/lib/aulaAiPrompts.js`); twarde limity (max 30 wiadomości kontekstu, 4000 chars/msg, 6000 chars single text, 500 output tokens). Stream przez `GroqProvider.sendMessage` 1:1 (model `qwen/qwen3-32b` z env override) + TransformStream strip `<think>...</think>` w locie (defensive vs Qwen3 reasoning leak — pierwsza warstwa to `reasoning_format: 'hidden'` w `GroqProvider`). Auth: opcjonalny Bearer JWT (anonim też ma rate-limit). Rate limit: **bardziej restrykcyjny niż /api/chat** — 10 req/min per user (refill 6s); klucz `aula-ai:<userId>` lub `aula-ai:ip:<ip>` dla anon. Brak Function Calling / tools — kontekst dostarcza klient w `input`.
- **`AulaAiService` + `AiInsightModal`** — klient [src/services/ai/AulaAiService.ts](src/services/ai/AulaAiService.ts) wystawia 4 metody AsyncGenerator (`summarizeChannel/explainText/simplifyText/translateText`), parser SSE niezależny od `BielikAdapter` (mimo identycznego formatu — żeby świadomie nie łączyć dwóch endpointów). [AiInsightModal](src/components/aula/AiInsightModal.tsx) to uniwersalny modal sterowany **factory function** (`start: () => AsyncGenerator<string>`) zamiast gotowym generatorem — pozwala "Spróbuj jeszcze raz" wywołać fresh stream bez re-mountu komponentu. Render przez `TypewriterMarkdown` (reused z `/chat`). `attemptRef` chroni przed race condition gdy user kliknie Retry mid-stream (stare delty są dropowane przez `myAttempt !== attemptRef.current` guard). UX: bottom-sheet na mobile (drag-to-close), centered modal na desktop, ESC + backdrop close, `navigator.clipboard.writeText` dla "Kopiuj".
- **AI entry points w UI** — [ChannelHeader](src/components/aula/ChannelHeader.tsx) ma opcjonalny `onSummarizeAi` (Sparkles button obok mute/notes/tasks); [AulaMessageItem](src/components/aula/AulaMessageItem.tsx) ma opcjonalny `onAiAction` (hover-action button "AI" + inline dropdown z 3 opcjami: Wyjaśnij prościej / Streść w 2 zdaniach / Przetłumacz na EN). Oba handlery zdefiniowane w [AulaView](src/components/aula/AulaView.tsx) ustawiają wspólny `aiModalConfig` state — jedna kolejka modali, każdy task ma własny `title + subtitle + start`. Subtitle dla summarize zawiera `CHANNEL_KIND_META[kind].long` (Wykład/Ćwiczenia/...); dla per-message — pierwsze 90 chars treści jako preview. Snapshot dla summarize filtruje `deleted_at` i puste contenty, bierze ostatnie 30 wiadomości, mapuje przez `userNames` na `authorName`. **`onAiAction` przekazywany rekurencyjnie do children** — AI actions działają na każdej głębokości wątku.
- **Smart `/chat` composer (voice + slash + stop)** — flagowy asystent w [ChatHubView](src/components/chat/ChatHubView.tsx) i wyspa [ChatAssistant](src/components/chat/ChatAssistant.tsx) dostały trzy upgrade'y composera. **Voice input** ([ChatVoiceButton](src/components/chat/ChatVoiceButton.tsx)) reusu'je `useVoiceRecorder` (ten sam co Aula voice notes) → po stop blob leci do [api/transcribe.ts](api/transcribe.ts) (Edge wrapper na Groq `whisper-large-v3-turbo`, 20 req/min per user, hard cap 25 MB / 60s, walidacja MIME whitelist + Bearer auth opcjonalny). Transkrypt wstaje w composer **bez auto-sendu** — user może doszlifować przed wysłaniem (krytyczne dla PL i nazw własnych typu WZIKS/MISH). **Slash commands** ([src/lib/chatSlashCommands.ts](src/lib/chatSlashCommands.ts)) — 7 preset queries (4 pierwsze IDENTYCZNE z `QUICK_PROMPTS` w ChatAssistant/ChatHubView — wspólny `responseCache` hit), `isSlashMode(draft)` true gdy draft zaczyna się od `/` i nie ma whitespace, `filterSlashCommands` sortuje slug-prefix > slug-substring > label-substring. [SlashCommandMenu](src/components/chat/SlashCommandMenu.tsx) renderuje się `absolute bottom-full` nad composerem (parent wrapper musi być `relative`); keyboard: ↑/↓ nawigują, Enter wybiera, Tab uzupełnia `/<slug> `, Esc kasuje draft. **Stop button** — gdy `isTyping`, Send zamienia się w czerwony Square triggerujący `cancel()` z `useChatSend` (AbortController już istniał — nie ma nowych race conditions). Slash menu TYLKO w `/chat` (wyspa za mała), voice + stop w obu powierzchniach.
- **Personal Function Calling tools** ([api/_lib/tools/getMyUserContext.ts](api/_lib/tools/getMyUserContext.ts), [getMyAulaOverview.ts](api/_lib/tools/getMyAulaOverview.ts), [findUser.ts](api/_lib/tools/findUser.ts)) — trzy user-scoped tools dla `/api/chat`. **`get_my_user_context`** czyta `profiles` (imię, kierunek, year_started → liczy aktualny rok studiów regułą PL "rok akademicki od 1 października", tryb, wydział, bio≤160), `auth: required`. **`get_my_aula_overview`** = personal dashboard Auli: cohort name + members count, top 5 otwartych zadań (`cohort_channel_tasks.completed_at IS NULL` ∩ NOT IN `cohort_task_completions` for me, sort `due_at` ASC NULLS LAST), aktywne polle (`cohort_message_polls.closed_at IS NULL` ∩ NOT IN `cohort_poll_votes` for me) z embedded `cohort_messages!inner` dla channel context; 7 round-tripów ale 6 paralelne przez `Promise.all`; format daty PL ("dziś 18:00" / "jutro 09:00" / "X dni po terminie ⚠️"). **`find_user`** wyszukuje `profiles` przez `or(full_name.ilike,username.ilike,study_program.ilike)`, filtruje `is_searchable!=false` + `is_banned!=true` + skip self, JS-side score (exact username > prefix > substring > program), top 5 z deep-linkami `[Name](/u/username)`. Wszystkie trzy zwracają **markdown STRING** (przepuszczany 1:1 przez `formatToolResultAsFinalAnswer` `typeof === 'string'` guard — zero formatter switch potrzebnego); TTL w [api/_lib/cache.ts](api/_lib/cache.ts): user_context 300s, aula_overview 30s, find_user 60s. Anon dostaje przyjazny "Musisz być zalogowany" zamiast 500.
