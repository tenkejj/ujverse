...
const MOCK_WZIKS_CLUBS: StudentClub[] = [
    ...
    <>
      <section className="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-950">
        ...
        <div className="space-y-3">
          {MOCK_WZIKS_CLUBS.slice(0, 3).map((club) => (
            <button
              type="button"
              key={club.id}
            ...
            >
              <div className="min-w-0 flex-1">
                ...
                <p className={`mt-0.5 text-left text-xs ${sideMutedCls}`}>
                  {club.department}
                  <span className="ml-1 text-zinc-500 dark:text-zinc-500">{club.category}</span>
                </p>
              </div>
            </button>
          ))}
      ...
      {isCreateModalOpen &&
        createPortal(
          ...
          >
            <div className="w-full max-w-lg rounded-2xl border border-zinc-200 dark:border-white/10 bg-white/80 dark:bg-black/40 p-5 shadow-2xl backdrop-blur-md">
              <div className="mb-4 flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <h3 className="text-base font-extrabold tracking-wide text-brand-gold [text-shadow:0_0_10px_rgba(201,162,39,0.25)]">Proces zakładania koła</h3>
              ...
              <p className="text-sm text-zinc-900 dark:text-zinc-100 leading-relaxed">
                Proces zakładania koła: 1. Weryfikacja pomysłu, 2. Znalezienie opiekuna, 3. Zatwierdzenie przez Dziekanat.
              </p>
      ...
      {selectedClub &&
        createPortal(
          ...
          >
            <div className="w-full max-w-md rounded-2xl border border-zinc-200 dark:border-white/10 bg-white/80 dark:bg-black/40 p-5 shadow-2xl backdrop-blur-md">
              <div className="mb-3 flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <h3 className="text-sm font-extrabold tracking-wide text-brand-gold [text-shadow:0_0_10px_rgba(201,162,39,0.25)]">{selectedClub.name}</h3>
                  <p className="mt-1 text-xs text-zinc-600 dark:text-zinc-400">{selectedClub.department}</p>
                </div>
                <button
                  type="button"
