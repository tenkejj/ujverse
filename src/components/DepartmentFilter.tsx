...
export default function DepartmentFilter({ selected, onChange }: Props) {
  ...
  return (
    <div className="relative w-full">
      <div
        ref={containerRef}
        className="m-0 flex w-full min-w-0 max-w-full flex-nowrap justify-start gap-2 overflow-x-auto overscroll-x-contain [touch-action:pan-x] scrollbar-hide [-webkit-overflow-scrolling:touch] [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:[display:none] py-0 px-4 md:px-6"
      >
        <button
          type="button"
          onClick={() => onChange('')}
